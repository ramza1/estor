class Admin::History::AddressesController < Admin::BaseController
  ...
  def index
    @order = Order.includes({:customer => :addresses}).find_by_number(params[:order_id])
  ...
  def create  ##  This create a new address, sets the orders address & redirects to order_history
    @order    = Order.includes([:ship_address, {:customer => :addresses}]).find_by_number(params[:order_id])
    @address  = @order.customer.addresses.new(allowed_params)
