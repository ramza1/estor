class Shopping::BaseController < ApplicationController
  ...
  def next_form(order)
    ...
    ## If we are insecure
      ...
      #return new_customer_session_url()
  ...
  def not_secure?
    !current_customer || has_not_logged_in_recently? || user_visited_a_non_ssl_page_since_login?
  end
  ...
  def find_or_create_order
    ...
    if session[:order_id]
      @session_order = current_customer.orders.include_checkout_objects.find(session[:order_id])
  ...
  def create_order
    @session_order = current_customer.orders.create(:number       => Time.now.to_i,
                                                :ip_address   => request.env['REMOTE_ADDR'],
                                                :bill_address => current_customer.billing_address  )
