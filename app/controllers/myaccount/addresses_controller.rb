class Myaccount::AddressesController < Myaccount::BaseController
  ...
  def new
    ...
    @address.default = true          if current_customer.default_shipping_address.nil?
    @form_address = @address
  end

  def create
    @address = current_customer.addresses.new(allowed_params)
    @address.default = true          if current_customer.default_shipping_address.nil?
    @address.billing_default = true  if current_customer.default_billing_address.nil?
  ...
  def edit
    form_info
    @form_address = @address = current_customer.addresses.find(params[:id])
  end
  ...
  def update
    ...
    @address.default         = true if params[:id].to_i == current_customer.default_shipping_address.try(:id)
    @address.billing_default = true if params[:id].to_i == current_customer.default_billing_address.try(:id)

    respond_to do |format|
      if @address.save
        format.html { redirect_to(myaccount_addresses_url, :notice => 'Address was successfully updated.') }
      else
        # the form needs to have an id
        @form_address = current_custoemr.addresses.find(params[:id])
