...
class Order < ActiveRecord::Base
  ...
  def remove_user_store_credits
    customer.store_credit.remove_credit(amount_to_credit) if amount_to_credit > 0.0
  end
  ...
  def self.find_finished_order_grid(params = {})
    grid = Order.includes([:customer]).where("orders.completed_at IS NOT NULL")
  ...
  # @return [none]
  def set_email
    self.email = customer.email if customer_id
  end
